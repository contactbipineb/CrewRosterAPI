...
namespace EY.CabinCrew.Repositories
{
    ...
    public class DatabaseCrewRosterRepository : IRepository<Roster>
    {
        ...
        private async Task<PersonalDetails> GetPersonalDetails(string employeeId)
        {
            ...
            // date
            string commandText = "SELECT top 100 Name,rank as Contact,CrewType as Address FROM DBO.VW_PERSONALDETAILS where personal_id= @CrewId";
            //string sql = @"select top 1  Name,rank as Contact  from DBO.VW_PERSONALDETAILS where personal_id=@PersonalId";
        ...
        private async Task<List<Plan>> GetPlans(string employeeId)
        {
            ...
            var parameters = new { CrewId = employeeId };
            string commandText = "SELECT TOP 100 [PK_Plan] as Id,[Month],[WeekNumber],[DayOfWeek] as Day FROM [DBO].[VW_PLAN] where PK_Plan = @CrewId";
            //string sql = @"select top 1  Name,rank as Contact  from DBO.VW_PERSONALDETAILS where personal_id=@PersonalId";
        ...
        private async Task<List<IataCountryCode>> GetIataCurrencies (string source, string destination)
        {
            ...
            string commandText = "SELECT [Iata], [CurrencyCode] FROM [DBO].[IataCurrency] where [Iata]= @Source OR [Iata]= @Destination";
        ...
        private async Task<IataCountryCode> GetIataCurrency(string iata)
        {
            ...
            string commandText = "SELECT [Iata], [CurrencyCode] FROM [DBO].[IataCurrency] where [Iata]= @Iata";
